...
def get_system_stats():
    ...
    return {
        "os": "Android/Termux"
        if "Android" in platform.system() or os.path.exists("/system/build.prop")
...
def analyze_history():
    ...
    try:
        ...
        cmds = (
            re.findall(r"^: \d+:\d+;(.*?)(?:\s|$)", content, re.MULTILINE)
            or content.splitlines()
        )
        ...
        if aliases_suggestion:
            ...
            console.print(
                "[grey]Tu flujo es eficiente. No se detectaron comandos largos repetitivos.[/]"
            )
...
def list_themes():
    ...
    table = Table(
        title=f"ARSENAL COMPLETO ({len(sorted_themes)} temas)", box=box.ROUNDED
    )
    ...
    console.print(table)
    console.print(
        "[yellow]Para usar uno:[/] Edita ~/.zshrc y cambia ZSH_THEME='tu_tema'"
    )
...
def self_update():
    ...
    console.print(
        "[italic grey]Esto descarga las últimas mejoras, temas God Tier y correcciones del repositorio oficial.[/]\n"
    )

    repo_dir = PROJECT_ROOT
    if not (repo_dir / ".git").exists():
        console.print(
            "[red]Error: No se detectó un repositorio git. ¿Instalaste manualmente?[/]"
        )
        return
    ...
    try:
        ...
        if res.returncode == 0:
            ...
            if "Already up to date" in res.stdout:
                ...
                console.print(
                    "[bold yellow]⚠ Cambios detectados. Se recomienda reiniciar la terminal o ejecutar ./install.sh[/]"
                )
        else:
...
def show_plugins_detail():
    ...
    console.print(
        f"[bold green]Detectados {len(plugins)} plugins activos en .zshrc[/]\n"
    )
    for p_name in plugins:
        ...
        if not info["found"]:
            console.print(
                Panel(
                    "No se encontró el archivo fuente (posible plugin nativo de OMZ sin script .zsh).",
                    title=title,
                    border_style="grey39",
                )
            )
        ...
        if info["aliases"]:
            content.append(
                f"[bold yellow]Alias ({len(info['aliases'])}):[/] "
                + ", ".join(info["aliases"][:10])
                + ("..." if len(info["aliases"]) > 10 else "")
            )
        if info["functions"]:
            content.append(
                f"[bold green]Funciones ({len(info['functions'])}):[/] "
                + ", ".join(info["functions"][:5])
                + ("..." if len(info["functions"]) > 5 else "")
            )

        if not content:
            content = [
                "[italic grey]Este plugin no exporta alias o funciones explícitas.[/]"
            ]
