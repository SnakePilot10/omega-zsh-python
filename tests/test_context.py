...
def test_detect_termux(mock_check, mock_split, mock_platform):
    ...
    with patch.dict("os.environ", env, clear=True), \
         patch("shutil.which", side_effect=lambda x: "/usr/bin/nala" if x == "nala" else None):
        ctx = SystemContext()
...
def test_detect_linux_debian(mock_platform):
    ...
    with patch("pathlib.Path.exists", mock_exists), \
         patch("builtins.open", MagicMock(side_effect=[MagicMock(__enter__=lambda s: ["ID=debian\n", "VERSION_ID=12\n"])])):
...
def test_detect_arch_linux(mock_platform):
    ...
    with patch.dict("os.environ", {}, clear=True), \
         ...
         patch("builtins.open", MagicMock(side_effect=[MagicMock(__enter__=lambda s: ["ID=arch\n"])])):
...
def test_detect_fedora(mock_platform):
    ...
    with patch.dict("os.environ", {}, clear=True), \
         patch("pathlib.Path.exists", return_value=True), \
         patch("builtins.open", MagicMock(side_effect=[MagicMock(__enter__=lambda s: ["ID=fedora\n"])])):
...
def test_detect_fallback(mock_platform):
    ...
    with patch.dict("os.environ", {}, clear=True), \
        ...
        with patch("shutil.which", side_effect=lambda x: "/usr/bin/apk" if x == "apk" else None):
            ctx = SystemContext()
