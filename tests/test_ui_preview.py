...
class TestThemePreview(unittest.IsolatedAsyncioTestCase):
    async def test_preview_renderiza_ansi_correctamente(self):
        """Verifica que el output ANSI del subproceso se parsea como Rich Text."""
        with patch("omega_zsh.ui.screens.shutil.which", return_value="/bin/zsh"):
            with patch("omega_zsh.ui.screens.subprocess.run") as mock_run:
                ...
                theme = MagicMock(
                    id="test_theme",
                    path="/path/to/theme.zsh-theme",
                    desc="Test Origin",
                )

                async with OmegaApp().run_test(
                    initial_screen=ThemeSelectScreen([theme], "test_theme")
                ) as pilot:
                    ...
                    assert isinstance(update_arg, Text)
